ReaperControl{
    ...
    *send{|msgPath ... args|
        netAddr.sendMsg(*([msgPath] ++ args))
    }

    *action{|actionNum|
        this.send("/action", actionNum)
    }
    ...
    *goToProjectEnd{
        this.send("/action", 40043);
    }

    *setTempo{|bpm, syncDefaultTempoClock=false, includeTempoMarker=true|

        // Set SC's tempo clock to the same as Reaper
        if(syncDefaultTempoClock, {
            TempoClock.default.tempo_(bpm / 60);
        });

        this.send("/tempo/raw", bpm);

        if(includeTempoMarker, {
            this.setTempoMarkerAtCursor();
        })
    }

    *setTempoMarkerAtCursor{
        this.send("/action", 42330);
    }

    *setMarker{
        this.send("/action", 40157);
    }

    *setNameOfLastMarker{|string|
        this.send("/lastmarker/name", string.asString);
    }

    *setAndNameMarker{|string|
        this.setMarker();
        this.setAndNameMarker(string);
    }

    // sendAmount is 0.0 to 1.0
    *setTrackSendVolume{|trackNum, sendNum, sendAmount|
        this.send("/track/%/send/%/volume".format(trackNum, sendNum), sendAmount);
    }

    *setTrackVolume{|trackNum, volume|
        this.send("/track/%/volume".format(trackNum), volume);
    }

    *setTrackPan{|trackNum, pan|
        this.send("/track/%/pan".format(trackNum), pan);
    }
